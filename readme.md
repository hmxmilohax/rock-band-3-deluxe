...
Then, navigate to `windows_bats` and **run `build_xenia.bat` to automatically update, build, and run Rock Band 3 Deluxe.** You need to run this script every time you want to play. `run_xenia.bat`, however, will not automatically build and update the game and will only run it.
...
Then, if you set up the repo, navigate to `windows_bats` and **run `build_ps3.bat`**.
...
Then, if you set up the repo, navigate to `windows_bats` and **run `build_xbox.bat`**.
