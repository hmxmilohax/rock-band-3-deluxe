...
for pro_song in rb3_plus_path.glob("Pro Strings/*/*"):
    ...
    for pro_file in pro_song.glob("*"):
        ...
        if pro_file.name == "upgrades.dta":
            ...
            merged_songs[pro_song.stem]["rank"].pop("keys", None)
            merged_songs[pro_song.stem]["rank"].pop("real_keys", None)
...
with open(root_dir.joinpath(f"_ark/songs/dta_sections/rb3_plus_strings.dta"), "w") as fff:
...
# uncomment rb3 plus strings.dta
overwritten = False
for i in range(len(missing_song_data)):
    if ";#include dta_sections/rb3_plus_strings.dta" in missing_song_data[i]:
